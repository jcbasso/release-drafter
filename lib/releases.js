...
const generateReleaseInfo = ({
  ...
  // --- New arguments ---
  prNumber,
  draftRelease, // The generic draft found by findReleases
  // --- End new arguments ---
}) => {
  ...
  if (isPrereleaseRun && draftRelease) {
    // Check if the found draftRelease belongs to *this* PR
    const bodyMatch = draftRelease.body?.match(PR_MARKER_REGEX)
    if (bodyMatch && Number.parseInt(bodyMatch[1], 10) === prNumber) {
      prSpecificDraftExists = true
      log({
        context,
        message: `Found existing draft release (${draftRelease.tag_name}) for PR #${prNumber}. Incrementing suffix.`,
      })

      try {
        const currentTag = draftRelease.tag_name
        ...
        if (
          ...
          if (typeof currentSuffix === 'number') {
            ...
            const simpleVersionInfo = {
              ...
              // Add dummy values for others if needed by templates, or adjust templates
              // Alternatively, create a more complete versionInfo structure if required
            }

            // Recalculate tag and name based *only* on the incremented version if not explicitly provided
            if (calculatedTag === undefined) {
              calculatedTag = template(tagTemplate || '', simpleVersionInfo)
              ...
              calculatedTag = template(calculatedTag, simpleVersionInfo)
            }

            // eslint-disable-next-line unicorn/prefer-ternary
            if (calculatedName === undefined) {
              calculatedName = template(nameTemplate || '', simpleVersionInfo)
            } else {
              // template explicit name with new version info
              calculatedName = template(calculatedName, simpleVersionInfo)
            }
            ...
            versionInfo = simpleVersionInfo // Use the minimal info we derived
          } else {
            log({
              context,
              message: `Could not parse numeric suffix in draft tag ${currentTag}. Falling back to standard versioning.`,
            })
          ...
          log({
            context,
            message: `Draft tag ${currentTag} does not match expected prerelease format. Falling back to standard versioning.`,
          })
          prSpecificDraftExists = false // Treat as if no draft found
        }
      } catch (error) {
        log({
          context,
          error,
          message: `Error processing draft tag ${draftRelease.tag_name}. Falling back to standard versioning.`,
        })
        prSpecificDraftExists = false // Treat as if no draft found
      }
    } else {
      log({
        context,
        message: `Found draft ${draftRelease.tag_name}, but it's not for PR #${prNumber}. Will create a new prerelease.`,
      })
    }
  }
  ...
  if (versionInfo === null) {
    const versionKeyIncrement = resolveVersionKeyIncrement(
      mergedPullRequests,
      config,
      // Pass the effective prerelease status for determining increment type (e.g., 'prepatch')
      isPreRelease
    )
  ...
  generatedBody = template(
    generatedBody,
    {
      ...
      $OWNER: owner,
      $REPOSITORY: repo,
      // Merge versionInfo here so body template also has version vars
      ...versionInfo,
    },
    replacers
  )
