...
const generateReleaseInfo = ({
  ...
  if (isPrereleaseRun && prSpecificRelease) {
    ...
    const bodyMatch = prSpecificRelease.body?.match(PR_MARKER_REGEX)
    if (bodyMatch && Number.parseInt(bodyMatch[1], 10) === prNumber) {
      ...
      log({
        context,
        message: `Found draft ${prSpecificRelease.tag_name}, but it's not for PR #${prNumber}. Will create a new prerelease.`,
      })
    }
  }
